...
module.exports = function (app) {
  ...
  routerV2.post('/uncompress', uncompress);
...
// Utility function to uncompress gzip data after retrieval
function fnUncompress(buffer) {
  ...
  try {
    result = JSON.parse(zlib.gunzipSync(new Buffer(buffer, 'base64')));
  } catch (e) {
    result = null;
  }
...
function uncompress(req, res, next) {
  console.log('req.body', req.body);
  let text = req.body.text;
  res.send(fnUncompress(text));
}